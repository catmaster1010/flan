...
static madt_ioapic_t *ioapic_from_gsi(uint32_t gsi) {
    for (uint64_t i = 0; i < madt_ioapics.items; i++) {
        ...
        if (gsi >= io_apic->gsib &&
            gsi < io_apic->gsib + ioapic_gsi_count(io_apic)) {
            return io_apic;
        }
    }
...
void ioapic_redirect_gsi(uint32_t lapic_id, uint32_t gsi, uint8_t vector,
                         uint64_t flags) {
    uint64_t ioredtbl_data =
        ((uint64_t)lapic_id << 56) | ((uint64_t)vector) | flags;
    madt_ioapic_t *ioapic = ioapic_from_gsi(gsi);
    uint64_t ioredtbl = (gsi - ioapic->gsib) * 2 + 0x10;
    ioapic_write(ioapic->address, ioredtbl, (uint32_t)ioredtbl_data);
    ioapic_write(ioapic->address, ioredtbl + 1,
                 (uint32_t)(ioredtbl_data >> 32));
}
...
void ioapic_redirect_irq(uint32_t lapic_id, uint8_t irq, uint8_t vector,
    ...
    for (uint64_t i = 0; i < madt_isos.items; i++) {
        ...
        if (iso->irq_source == irq) {
            ioapic_redirect_gsi(lapic_id, iso->gsi, vector,
